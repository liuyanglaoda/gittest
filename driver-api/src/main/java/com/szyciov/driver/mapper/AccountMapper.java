...
public interface AccountMapper {
	/**
	 * 查询司机实际所分配车型(根据订单归属方)
	 * @param driver  需要id跟leasecompanyid
	 * @return
	 */
	PubDriver getPubDriverFactModel(PubDriver driver);
	/**
	 * 根据账户密码查询司机
	 * @param param
	 * @return
	...
	@Select("select pc.address,pd.name curDriverName,pd.jobnum curDriverCertNo,pv.fullplateno curVehicleNo,pv.certificate curVehicleCertNo,DATE_FORMAT(pd.uptime,'%Y%m%d%H%i%s') curTimeOn,DATE_FORMAT(NOW(),'%Y%m%d%H%i%s') curTimeOff,CONCAT(pd.name,'主班') reportDrivers,pvd.jobnum reportDriverCertNo,pv.fullplateno reportVehicleNo,pvd.jobnum reportVehicleCertNo,DATE_FORMAT(pvd.contracton,'%Y%m%d%H%i%s') timeFrom,DATE_FORMAT(pvd.contractoff,'%Y%m%d%H%i%s') timeTo " +
			"from pub_driver pd " +
			"LEFT JOIN pub_driver_vehicle_bind dvr ON dvr.driverid = pd.id " +
			"LEFT JOIN pub_vehicle pv ON pv.id = dvr.vehicleid " +
			"LEFT JOIN pub_driver pvd ON pvd.id = pv.driverid " +
			"LEFT JOIN pub_cityaddr pc on pc.id = pv.address " +
			"where pd.id = #{id} order by pd.uptime desc limit 1")
	Map<String,String> peopleVehicleRalation(String id);
}
