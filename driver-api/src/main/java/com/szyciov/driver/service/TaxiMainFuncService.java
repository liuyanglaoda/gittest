...
//		String companyname = companyinfo==null?"车企":companyinfo.getShortName();
	...
	private boolean doPayCallback(PubDriverTradingrecord record){
		...
		if(PayUtilEnum.RECHARGE.code.equals(record.getType() + "")){
			...
			if(OrderEnum.ORDERTYPE_TAXI.code.equals(record.getOrdertype())){
				doPayOrderCallback(record);                //出租车订单支付回调
			}else{
				doPayNetCarOrderCallback(record);    //网约车订单支付回调
			}
		}else{
     ...
     * @return
     */
    private boolean doPayNetCarOrderCallback(PubDriverTradingrecord record){
		logger.info("网约车订单支付回调...");
		String companyid = null;
		//订单支付则改变订单状态
		if(OrderEnum.USETYPE_PERSONAL.code.equals(record.getUsetype())){
			OpOrder opOrder = orderDao.getOpOrder(record.getOrderno());
			companyid = opOrder.getCompanyid();
			opOrder.setOrderstatus(OrderState.SERVICEDONE.state);
			opOrder.setPaymentstatus(PayState.STATEMENTED.state);
			if(record.getPaymenttype() == 1){
				opOrder.setPaytype(OrderEnum.PAYTYPE_WECHAT.code);
			}else{
				opOrder.setPaytype(OrderEnum.PAYTYPE_ALIPAY.code);
			}
			//支付完成,改变订单状态,司机需要支付只可能是线下支付
			opOrder.setCompletetime(new Date());    //订单完结时间
			orderDao.updateOpOrder(opOrder);
		}else{
			OrgOrder orgOrder = orderDao.getOrgOrder(record.getOrderno());
			companyid = orgOrder.getCompanyid();
			orgOrder.setOrderstatus(OrderState.SERVICEDONE.state);
			//支付完成,改变订单状态,司机需要支付只可能是线下支付
			if(PayState.MENTED.state.equals(orgOrder.getPaymentstatus())){
				orgOrder.setPaymentstatus(PayState.STATEMENTED.state);
			}else{
				if(record.getPaymenttype() == 1){
					orgOrder.setPaytype(OrderEnum.PAYTYPE_WECHAT.code);
				}else{
					orgOrder.setPaytype(OrderEnum.PAYTYPE_ALIPAY.code);
				}
				orgOrder.setPaymentstatus(PayState.PAYED.state);
			}
			orgOrder.setCompletetime(new Date());    //订单完结时间
			orderDao.updateOrgOrder(orgOrder);
		}
		...
		DriverCollectionCashDto dto = new DriverCollectionCashDto();
		dto.setOrderno(record.getOrderno());
		dto.setOrdertype(record.getOrdertype());
		dto.setUsetype(record.getUsetype());
		doConfirmCost4Advance(dto,companyid);   //网约车暂时只有个人垫付会用到此回调
		//司机累计接单量+1(完成时才算)
	 ...
	 * @param dto
	 */
	private void doConfirmCost4Advance(DriverCollectionCashDto dto,String companyid){
		OrderApiParam oap = new OrderApiParam();
		...
		doSaveDriverPayManagement(ccdto,companyid);
	}
	 ...
	 * @param param
	 */
	private void doSaveDriverPayManagement(ConfirmCostDto param,String companyid){
		//附加费为0时不保存司机垫付记录信息
	...
	private JSONArray doGetPayMethods(PurseParam param){
		...
		//支付方式(个人垫付,个人支付,机构支付)
		String paymethod = PayMethod.PERSONAL.code;
		if(order.get() instanceof OrgOrder){
			paymethod = ((OrgOrder)order.get()).getPaymethod();
		}
		//订单付款有余额渠道(个人垫付只能司机代付,没有余额支付渠道)
		if(PayUtilEnum.PAYORDER.code.equals(param.getType()) 
			&& !PayMethod.ADVANCE.code.equals(paymethod)){
...
//		if(purse.get() == null) return false;
     ...
     * @return
     */
    @ValidateRule("您有未结算订单，请结算完成后再提现")
