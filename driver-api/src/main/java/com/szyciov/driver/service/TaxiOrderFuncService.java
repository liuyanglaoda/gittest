...
public class TaxiOrderFuncService extends BaseService{
	...
	public void setOrderSurchargeDao(OrderSurchargeDao osDao){
		this.osDao=osDao;
	}
	...
	public JSONObject confirmCost(ConfirmCostDto param){
		String[] require = new String[]{
				"usetype","ordertype","orderno","orderstatus","paymethod",
	...
	public JSONObject taxiChangeOrderState(OrderApiParam param){
		...
		if(OrderState.START.state.equals(param.getOrderstate())){
			require = new String[]{"usetype","ordertype","orderno","orderstatus","hasearlierorder"};
		}
		if(!checkeParam(param,getExceptElement(param, require)) || order.get() == null) {
			return errorResult.get();
		}
	...
	private void doSaveSurcharge(ConfirmCostDto param){
		...
		try {
			...
			logger.error("保存订单附加费明细异常,当前参数" + StringUtil.parseBeanToJSON(param));
		}
...
//			taxiorderfuncdao.updateOrgTaxiOrder(taxiOrder);
	...
	private boolean doDriverStart(OrderApiParam param){
		...
		//司机状态缓存
		DriverCacheUtil.startOrder(driver.get().getId(), taxiOrder.getOrderno());
	...
	private boolean doDriverInservice(OrderApiParam param){
		...
		//司机状态缓存
		DriverCacheUtil.updateOrderStartTime(driver.get().getId(), taxiOrder.getOrderno(),taxiOrder.getStarttime());
	...
	private boolean doDriverWaitMoney(OrderApiParam param){
		...
		taxiOrder.setCosttype(CalcTypeEnum.LBSYUN.code);
		...
		OrderInfoDetail orderInfo = this.taxiorderfuncdao.getOpTaxiOrderInfoById(param);
		checkUserRegisterCity(orderInfo, orderInfo.getUserphone());  //检查用户最近5单是否同一城市
	 ...
	 * @param pd
	 */
	private void saveTakingOrderCache(OpTaxiOrder taxiOrder,PubDriver pd){
		OrderCacheInfoDTO cacheInfoDTO = new OrderCacheInfoDTO();
		cacheInfoDTO.setOrderNo(taxiOrder.getOrderno());
		cacheInfoDTO.setUseNow(taxiOrder.isIsusenow());
		cacheInfoDTO.setUseTime(StringUtil.formatDate(taxiOrder.getUsetime(), StringUtil.DATE_TIME_FORMAT));
		cacheInfoDTO.setEstimatedtime((long)taxiOrder.getEstimatedtime());
	 ...
	 * @param userPhone
	 */
	private void checkUserRegisterCity(OrderInfoDetail orderInfo,String userPhone){
		try{
			PeUser user = getPeUser(userPhone);
			if(null == user.getRegistercity()){
				user.setRegistercity(orderInfo.getCityid());
			}else if(checkUserLastOrders(userPhone)){
				user.setRegistercity(orderInfo.getCityid());
			}
			taxiorderfuncdao.updatePeUserCity(user);
		}catch(Exception e){
	...
	private boolean checkUserLastOrders(String userPhone){
		List<OrderInfoDetail> list = taxiorderfuncdao.getLastOrders(userPhone);
	...
	private PeUser getPeUser(String userPhone){
		...
		user = taxiorderfuncdao.getPeUserByPhone(user);
//		if(user == null){
...
//		}
	...
	private boolean doTaxiTakingOrder(OrderApiParam param){
		...
		if(order.get() == null){
			...
			//司机状态缓存
			saveTakingOrderCache(taxiOrder,driver.get());
		}
		logger.info("订单抢单完成");
		return true;
	}
	 ...
	 * @return
	 */
	private boolean doConfirmCost(ConfirmCostDto param){
		logger.info("订单确费开始...");
		logger.info("使用参数:" + JSONObject.fromObject(param));
		OpTaxiOrder taxiOrder = (OpTaxiOrder)order.get();
		TaxiOrderCost cost = StringUtil.parseJSONToBean(taxiOrder.getPricecopy(), TaxiOrderCost.class);
		cost.setCost(param.getRangefee() + param.getSurchargefee());
		cost.setRangecost(param.getRangefee());
		cost.setSurcharge(param.getSurchargefee());
		taxiOrder.setPricecopy(StringUtil.parseBeanToJSON(cost));
		taxiOrder.setShouldpayamount(param.getRangefee() + param.getSurchargefee());
		taxiOrder.setActualpayamount(param.getRangefee() + param.getSurchargefee());
		taxiOrder.setOriginalorderamount(param.getRangefee() + param.getSurchargefee());
		taxiOrder.setOrderamount(param.getRangefee() + param.getSurchargefee());
		taxiOrder.setPayroutemoney(param.getRangefee());
		taxiOrder.setShouldroutemoney(param.getRangefee());
		taxiOrder.setPaysurcharge(param.getSurchargefee());
		taxiOrder.setShouldsurcharge(param.getSurchargefee());
		...
		taxiOrder.setExpensetype(Integer.parseInt(OrderEnum.EXPENSETYPE_SERVICE_DONE.code));
		//改变订单支付状态(逻辑太长,拆分)
...
//		if(changeOrderPayStatus(taxiOrder,param)) doAwardPoint(taxiOrder,param);
		...
		accdao.updatePubDriver(driver.get());
		//司机状态缓存
		DriverCacheUtil.endOrder(driver.get().getId(), taxiOrder.getOrderno());
		if(PayMethod.OFFLINE.code.equals(param.getPaymethod())){
	...
	private JSONObject doTaxiGetOrderDetail(OrderApiParam param){
		...
		List<String> urls = orderDao.getCertificateList(taxiOrder.getOrderno(), 2);
		...
		temp.put("isuploadcert", !urls.isEmpty());
	...
	private JSONObject convertOrder(OpTaxiOrder oo){
		...
		od.put("surchargecost", (oo.getPaysurcharge() == null ? 0 : oo.getPaysurcharge()));
//		od.put("schedulefee",StringUtil.formatNum(oo.getSchedulefee(), 1)+"");
		od.put("orderamount",StringUtil.formatNum(oo.getOrderamount() + oo.getSchedulefee() + oo.getPaysurcharge(), 1));
		od.put("remark", oo.getTripremark());
		od.put("status", oo.getOrderstatus());
		od.put("paymethod", oo.getPaymentmethod());
		int cancelAmount = (oo.getCancelamount() == null ? 0 : oo.getCancelamount());
		int cancelNature = (oo.getCancelnature() == null ? 2 : oo.getCancelnature());
		od.put("paystatus", StringUtil.formatTaxiPayStatus(oo.getPaymentstatus(), oo.getOrderstatus(),cancelAmount));
		...
		od.put("cancelamount", cancelAmount + "元");
		od.put("cancelnature", (1 == cancelNature ? "有责" : "免责"));
	 ...
	 * @param param
	 */
	private void savePeUserExpenses(ConfirmCostDto param){
	 ...
	 * @return 是否需要返点
	 */
	private boolean changeOrderPayStatus(OpTaxiOrder taxiOrder,ConfirmCostDto param){
		//如果是线下支付
		if(PayMethod.OFFLINE.code.equals(param.getPaymethod())){
			//如果调度费不为0,改为乘客未付
			if(taxiOrder.getSchedulefee() > 0){
			...
			//如果调度费为0,改为已结算(司机已收现,不再需要转给公司)
				...
				taxiOrder.setCosttype(CalcTypeEnum.LBSYUN.code);
				taxiOrder.setCompletetime(new Date());
				taxiOrder.setPaymentstatus(PayState.STATEMENTED.state);
				taxiOrder.setOrdersortcolumn(Integer.valueOf(OrdersortColumn.STATEMENTED.state));
				doUnreverseCoupon(taxiOrder);
				return true;
			}
		//如果是线上支付
		}else{ 
	 ...
	 * @param orderInfo
	 */
	private void doUnreverseCoupon(OpTaxiOrder taxiOrder){
		try {
			OrderCost oc = StringUtil.parseJSONToBean(taxiOrder.getPricecopy(), OrderCost.class);
			String url = SystemConfig.getSystemProperty("couponApi") + "/coupon/unreserve";
			Map<String,Object> map = new HashMap<>();
			map.put("oldCouponId", oc.getCouponid());
			map.put("orderId", taxiOrder.getOrderno());
			...
			if(Retcode.OK.code != couponResult.getStatus()) {
				logger.info("解锁优惠券失败:" + jsonStr);
			}else{
				logger.info("解锁优惠券成功:" + oc.getCouponprice() + " " + oc.getCouponid());
			}
		} catch (Exception e) {
			logger.error("解锁优惠券异常",e);
		}
	}
...
//				order.set(taxiorderfuncdao.getOrgTaxiOrder(oap));	//目前只有运管端有出租车
			...
			if(OrderEnum.USETYPE_PERSONAL.equals(usetype.get())){
				order.set(orderDao.getOpOrder(orderno));
			}else{
				order.set(orderDao.getOrgOrder(orderno));
			}
		}
		if(order.get() == null || !order.get().getDriverid().equals(driver.get().getId())){
			errorResult.get().put("status", Retcode.ORDERNOTEXIT.code);
			errorResult.get().put("message", Retcode.ORDERNOTEXIT.msg);
			return false;
		}
