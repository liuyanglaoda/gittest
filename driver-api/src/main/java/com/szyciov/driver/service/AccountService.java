...
import com.szyciov.dto.driver.PubDriverTimeCardDto;
...
import com.szyciov.enums.PubDriverTimeCardEnum;
...
public class AccountService extends BaseService{
	...
	public JSONObject login(LoginParam param) {
		...
		//如果使用短信验证码登录,并且验证不通过
		if(DriverEnum.LOGIN_TYPE_SMS.code.equals(param.getLogintype() + "") && !doCompairSMSCode(param,DriverEnum.SMS_TYPE_LOGIN)){
			return errorResult.get();
		//如果使用密码登录,并且验证不通过
...
//		param.setDriverid(driver.get().getId());
	...
	public JSONObject atwork(AtworkParam param) {
		...
		//原始上班状态(如果上班打卡时,司机本身处于上班状态,则不更新上班打卡记录)
		String rawWorkStatus = driver.get().getWorkstatus();
		param.setId(driver.get().getId());
		if(!doAtWork(param)) return errorResult.get();
		
		updateUptimeOrOnlinetime(param);
		//下班状态的司机,请求上班才更新打卡记录
		//本身是上班状态的司机再次请求上班,不更新打卡记录
		if(DriverEnum.ATWORK_TYPE_ON.code.equals(param.getType())){
			if(DriverState.OFFLINE.code.equals(rawWorkStatus) 
			|| DriverState.NOCAR.code.equals(rawWorkStatus)){
				saveDriverTimeCard(param);
			}
		}else if(DriverEnum.ATWORK_TYPE_OFF.code.equals(param.getType())){
			saveDriverTimeCard(param);
		}
		JSONObject result = new JSONObject();
		result.put("hoursonline", driver.get().getHoursonline());
		return result;
	}
	...
	public JSONObject sendSMS(SendSMSParam param) {
		...
		//如果是登录发送短信去掉token验证
//		boolean checkToken = DriverEnum.SMS_TYPE_CHANGEPASS.code.equals(param.getType());
	...
	public JSONObject authSMSCode(LoginParam param){
		...
		if(!checkeParam(param,getExceptElement(param, require,false))) return errorResult.get();
		JSONObject result = new JSONObject();
		if(!doCompairSMSCode(param,DriverEnum.SMS_TYPE_FORGETPASS)) {
			return errorResult.get();
		}
	...
	private boolean doAtWork(AtworkParam param){
		...
		String key="driverId_"+pd.getId()+"_onWorkTime";
		//上班设置为空闲,下班设置为离线
		if(DriverEnum.ATWORK_TYPE_ON.code.equals(param.getType())){
			pd.setWorkstatus(DriverState.IDLE.code);
			//司机状态缓存
			DriverCacheUtil.setStatus2Idle(param.getId());
//			司机上班
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			map.put("onWorkTime",nowtime);
			supervisionMessageUtil.sendToSupervision(InterfaceType.ORDER,CommandEnum.DriverOnWork,map);
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			map.put("loginTime",nowtime);
			map.put("longitude", String.valueOf(pd.getLng()));
			map.put("latitude", String.valueOf(pd.getLat()));
			supervisionMessageUtil.sendToSupervision(InterfaceType.OPERATION,CommandEnum.VehicleOperationOnline,map);
			//司机定位->上班
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			map.put("longitude", String.valueOf(pd.getLng()));
			map.put("latitude", String.valueOf(pd.getLat()));
			map.put("positionType", "SJSB");
			supervisionMessageUtil.sendToSupervision(InterfaceType.GPS, CommandEnum.DriverLocationInfo, map);
			//缓存上班时间
			try {
				JedisUtil.setString(key,nowtime,864000);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			pd.setWorkstatus(DriverState.OFFLINE.code);
			//司机状态缓存
			DriverCacheUtil.offline(param.getId());
			//司机下班
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			String onWorkTime=JedisUtil.getString(key);
			if(onWorkTime==null){
				onWorkTime=nowtime;
			}
			map.put("onWorkTime",onWorkTime);
			map.put("offWorkTime",getNowTime());
			supervisionMessageUtil.sendToSupervision(InterfaceType.ORDER,CommandEnum.DriverOffWork,map);
			//营运下线
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			map.put("logoutTime",nowtime);
			map.put("longitude", String.valueOf(pd.getLng()));
			map.put("latitude ", String.valueOf(pd.getLat()));
			supervisionMessageUtil.sendToSupervision(InterfaceType.OPERATION,CommandEnum.VehicleOperationOffline,map);
			// 处理监管平台，人车对应关系 begin
			Map<String, String>  data = dao.peopleVehicleRalation(pd.getId());
			//TODO 检查数据
//			MapUtil.checkMapValue(data, "信息不完整，请补充车辆或司机信息");
			supervisionMessageUtil.sendToSupervision(InterfaceType.RELATIONSHIP, CommandEnum.HumanVehicleInfo, data);
			//司机定位->下班
			map=new HashMap<>();
			map.put("driverId",pd.getId());
			map.put("longitude", String.valueOf(pd.getLng()));
			map.put("latitude", String.valueOf(pd.getLat()));
			map.put("positionType", "SJXB");
			supervisionMessageUtil.sendToSupervision(InterfaceType.GPS, CommandEnum.DriverLocationInfo, map);
	 ...
	 * @return
	 */
	private boolean doCompairSMSCode(LoginParam param,DriverEnum smsType){
		logger.info("校验短信验证码开始...");
		logger.info("使用参数:" + JSONObject.fromObject(param));
		SendSMSParam ssp = new SendSMSParam();
		ssp.setMobile(param.getMobile());
		ssp.setType(smsType.code);
	...
	private boolean doSendSMS(SendSMSParam param){
		...
		//忘记密码短信
		}else if (DriverEnum.SMS_TYPE_FORGETPASS.code.equals(param.getType())) {
			smsToken.setSmscode(StringUtil.getRandomNum(4));
			String content = SMSTempPropertyConfigurer.getSMSTemplate("com.szyciov.driver.service.accountservice.sendsms", smsToken.getSmscode());
...
//		di.setWorkstatus(driver.get().getWorkstatus());
	...
	private boolean doUploadOrdinates(UploadOrdinatesParam param){
		...
		map.put("driverId",pd.getId());
		map.put("ordertype", pd.getOrdertype());
		map.put("usetype", pd.getUsetype());
		map.put("latitude", String.valueOf(param.getLat()));
		map.put("longitude", String.valueOf(param.getLng()));
		supervisionMessageUtil.sendToSupervision(InterfaceType.GPS, CommandEnum.DriverLocationInfo, map);
		logger.info("发送监管参数:" + JSONObject.fromObject(map));
...
//		PlatformTypeByDb platform = OrderEnum.USETYPE_PERSONAL.equals(usetype.get()) ?PlatformTypeByDb.OPERATING:PlatformTypeByDb.LEASE;
		//2017年8月23日17:23:01 修改为司机端报警统一使用司机归属
		PlatformTypeByDb platform = PlatformTypeByDb.OPERATING.code.equals(driver.get().getPlatformtype()) ? PlatformTypeByDb.OPERATING : PlatformTypeByDb.LEASE;
	...
	private JSONObject doLogin(LoginParam param){
		...
		try{
			...
			map.put("driverId",driver.get().getId());
			supervisionMessageUtil.sendToSupervision(InterfaceType.BASIC,CommandEnum.DriverMobileTerminalInfo,map);
		}catch(Exception e){
			logger.error("驾驶员终端信息,推送至监管队列失败,driverid:"+driver.get().getId(), e);
		}
		return result;
	}
...
//			json.put("wdpchanged", "");
   ...
   private void updateUptimeOrOnlinetime(AtworkParam param){
		...
		if(DriverEnum.ATWORK_TYPE_ON.code.equals(param.getType())){
			...
			//TODO 处理监管平台，人车对应关系 begin
			Map<String, String>  data = dao.peopleVehicleRalation(pd.getId());
			SupervisionDto supervisionDto = new SupervisionDto();
			supervisionDto.setCommandEnum(CommandEnum.HumanVehicleInfo);
			supervisionDto.setInterfaceType(InterfaceType.RELATIONSHIP);
			supervisionDto.setDataMap(data);
    ...
    * @param param
    */
   private void saveDriverTimeCard(AtworkParam param){
	   try {
		   PubDriver pd = driver.get();
		   PubDriverTimeCardEnum opratetype = PubDriverTimeCardEnum.OPERATETYPE_INWORK;
		   PubDriverTimeCardEnum platfortype = PubDriverTimeCardEnum.PLATFORMTYPE_OPRATE;
		   if(DriverEnum.ATWORK_TYPE_OFF.code.equals(param.getType())){   //如果是请求下班改为下班打卡
			   opratetype = PubDriverTimeCardEnum.OPERATETYPE_OUTWORK;
		   }
		   if(PubDriverTimeCardEnum.PLATFORMTYPE_OPRATE.code != parseInt(pd.getPlatformtype())){  //如果是租赁司机改为租赁平台
			   platfortype = PubDriverTimeCardEnum.PLATFORMTYPE_LEASE;
		   }
		   PubDriverTimeCardDto dto = new PubDriverTimeCardDto();
		   dto.setDriverid(pd.getId());
		   dto.setVehicleid(pd.getVehicleid());
		   dto.setLeasescompanyid(pd.getLeasescompanyid());
		   dto.setOpratetype(opratetype);
		   dto.setPlatformtype(platfortype);
		   JSONObject result = templateHelper.dealRequestWithFullUrlToken(
			   carserviceApiUrl + "/Driver/SaveDriverTimeCard", HttpMethod.POST, null, dto, JSONObject.class);
		   if(Retcode.OK.code != result.getInt("status")){
			   logger.error("保存司机考勤打卡记录失败."+result.getInt("status")+":"+result.getString("message")+"");
		   }
	   } catch(Exception e) {
		   logger.error("保存司机考勤打卡记录异常.", e);
	   }
   }
	/**********************************************************校验方法***************************************************************/
	/**
	...
	private boolean checkAnotherLogin(PubUserToken token,String uuid){
		...
		if(uuid != null &&  token.getUuid() != null && !token.getUuid().equals(uuid)){
			...
			if(!(DriverState.NOCAR.code.equals(rawWorkStatus) 
			|| DriverState.OFFLINE.code.equals(rawWorkStatus))){
				//挤下线时更新在线时长和下班考勤,只有下班状态时才更新
				AtworkParam ap = new AtworkParam();
				ap.setType(DriverEnum.ATWORK_TYPE_OFF.code);
	...
	private boolean checkLoginPass(LoginParam param){
		...
		if(param.isEncrypted()){
			try {
				if(param.getPassword().length() != 344){
				...
				errorResult.get().put("status", Retcode.PASSWORDWRONG.code);
				errorResult.get().put("message", "解密失败,请检查密文");
				logger.info("RSA解密失败");
				return false;
			}
		}else{
	...
	private boolean onlineCarCheckInserviceOrder(OrderListParam olpParam){
		...
		if(list != null && !list.isEmpty()){
			if(OrderState.WAITMONEY.state.equals(list.get(0).getStatus())){
				errorResult.get().put("status", Retcode.HAS_NOT_CONFIRMCOST_ORDER.code);
				errorResult.get().put("message", Retcode.HAS_NOT_CONFIRMCOST_ORDER.msg);
			}else{
	...
	private boolean taxiCheckInserviceOrder(OrderListParam olpParam){
		...
		if(list != null && !list.isEmpty()){
			if(OrderState.WAITMONEY.state.equals(list.get(0).getOrderstatus())){
				errorResult.get().put("status", Retcode.HAS_NOT_CONFIRMCOST_ORDER.code);
				errorResult.get().put("message", Retcode.HAS_NOT_CONFIRMCOST_ORDER.msg);
			}else{
				errorResult.get().put("status", Retcode.ORDERNOTDONE.code);
	...
	private boolean checkCanAtWork(String type){
		...
		//网约车只检查是否存在服务中订单(含待确费订单)
        }else if (DriverEnum.DRIVER_TYPE_CAR.code.equals(driver.get().getVehicletype() + "") &&
        		DriverEnum.ATWORK_TYPE_OFF.code.equals(type)) {
			if(!checkInserviceOrder()){
				return false;
			}
    ...
    /**
     * 检查验证码错误次数
     * @param errorTimesSign
     * @return
     */
    @ValidateRule("验证码错误次数超限")
