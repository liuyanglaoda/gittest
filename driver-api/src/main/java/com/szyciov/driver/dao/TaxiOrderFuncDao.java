...
public class TaxiOrderFuncDao{
    ...
    public PeUser getPeUserByPhone(PeUser user){
    	PeUser p = this.taxiorderfuncmapper.getPeUserByPhone(user);
    	return p;
    }
     ...
     * @return
     */
    public OrderInfoDetail getOpTaxiOrderInfoById(OrderApiParam param){
    	return this.taxiorderfuncmapper.getOpTaxiOrderInfoById(param);
    }
    ...
    public List<OrderInfoDetail> getLastOrders(String userPhone){
    	return this.taxiorderfuncmapper.getLastOrders(userPhone);
    }
    /**
     * 更新个人用户的注册城市
     * @param user
    ...
    public void updatePeUserCity(PeUser user){
    	this.taxiorderfuncmapper.updatePeUserCity(user);
    }
}
